...
{	
	...
	private static final String[] English = {"Zero", "One", "Two", "Three", "Four", "Five", "Six", "Seven", "Eight", "Nine", "Ten"};
	private static final String[] Chinese = {"零","一","二","三","四","五","六","七","八","九","十"};
    ...
    public void start(Stage pPrimaryStage) 
    {
    	...
    	root.add(new TextPanel(model,Chinese), 0, 2, 1, 1);
    	root.add(new TextPanel(model,English), 0, 3, 1, 1);
...
{	//private static final String[] English = {"Zero", "One", "Two", "Three", "Four", "Five", "Six", "Seven", "Eight", "Nine", "Ten"};
	 ...
	 * @param pModel The model observed by this panel.
	 */
	TextPanel(Model pModel,String[] pLanguage)
	{	Labels = pLanguage;
		aModel = pModel;
		aModel.addObserver(this);
		aText.setMinWidth(LuckyNumber.WIDTH);
		aText.setText(pLanguage[aModel.getNumber()]);
		...
		aText.setOnAction(new EventHandler<ActionEvent>()
		{
			...
			{
				int lIndex = 0;
				for( int i = 0; i < pLanguage.length; i++)
				{
					if(aText.getText().equalsIgnoreCase(pLanguage[i]))
					{
						lIndex = i;
						break;
					}
