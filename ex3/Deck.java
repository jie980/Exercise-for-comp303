...
public class Deck implements CardSource, Iterable<Card>
{
	...
	public void shuffle()
	{
		...
		//aCards.add(new Card(Colour.black)); 因为excercise10 需要修改
		//aCards.add(new Card(Colour.white));
	 ...
	 */
	public void sort()
	{	//Collections.sort(aCards); 也可以 因为implement了comparable
		Collections.sort(aCards, Card.createByRankComparator());//可以不用implement comparable
		//方法2 
		/*Collections.sort(aCards,new Comparator<Card>()//匿名类，只实现一次,在card里被factory method替代
		{
			public int compare(Card pCard1, Card pCard2)
			{ 
				return pCard1.getRank().compareTo(pCard2.getRank());//enum里的compareTo不再局限于只对比rank
